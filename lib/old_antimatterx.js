(function(window, undefined) {
    ...
    window.old_antimatterx = {
        ...
        judgeType: function(x, typeName) { // xが指定された型名か判定
            var type = old_antimatterx.getType(x);
            switch (old_antimatterx.getType(typeName)) {
                case "String":
        ...
        init: function(param, defaultParam) { // キーの型が異なる場合defaultParamのキーで上書きして返す
            if (!old_antimatterx.judgeType(param, "Object")) param = {};
            for (var k in defaultParam) {
                var defaultType = old_antimatterx.getType(defaultParam[k]),
                    type = old_antimatterx.getType(param[k]);
        ...
        paddingArray: function(array, paddingChar, len) { // arrayの要素をpaddingCharでlenの長さに整形
            return array.map(function(v) {
                return old_antimatterx.paddingStr(v, paddingChar, len);
            });
        },
        ...
        randStr: function(table, num) { // ランダムな文字列を返す
            return old_antimatterx.makeArray(num).map(function() {
                return old_antimatterx.randArray(table.split(""));
            }).join("");
        },
        paddingStr: function(str, paddingChar, len) { // strをpaddingCharでlenの長さに整形
            return (old_antimatterx.repeat(paddingChar, len) + str).slice(-len);
        },
        ...
        BaseN: function(base) { // N進数を作成するクラス
            if (!old_antimatterx.judgeType(base, "String") || base.length < 2) return false;
            return {
        ...
        toBase58: { // 文字列を58進数にエンコード
            ...
            encode: function(str) { // 文字列 => 58進数
                var to58 = old_antimatterx.BaseN(old_antimatterx.makeBaseStr(58)),
                    sign = "WXYZ";
                if (!old_antimatterx.judgeType(str, "String")) return false;
                return str.split("").map(function(c) {
                    if (to58.base.indexOf(c) !== -1) return sign[0] + c + sign[0];
                        ...
                        return sign[len] + (old_antimatterx.repeat("0", len) + str).slice(-len) + sign[len];
                    };
        ...
        download: { // ファイルの保存
            file: function(filename, blob) { // blobからファイルを保存する
                if (!old_antimatterx.judgeType(filename, "String") || filename === "") return false;
            ...
            text: function(title, str) { // 文字列をテキストファイル形式で保存する
                if (!old_antimatterx.judgeType(title, "String") ||
                    !old_antimatterx.judgeType(str, "String") ||
                    title === "" ||
                    str === "") return false;
                old_antimatterx.download.file(title + ".txt", new Blob([
            ...
            image: function(title, dataURI) { // dataURIから画像を保存する
                if (!old_antimatterx.judgeType(title, "String") ||
                    !old_antimatterx.judgeType(dataURI, "String") ||
                    title === "" ||
                ...
                old_antimatterx.download.file(title + "." + m[1].split("/")[1], new Blob([cont], {
                    type: m[1]
                }));
            }
        },
        ...
        makeSaveKey: function(key) { // URLごとに保存する領域を分けるためのキーを作成
            if (!old_antimatterx.judgeType(key, "String") || key === "") return false;
        ...
        getSaveKeys: function() { // 保存されているキーを配列で取得
            ...
            if (window.localStorage) {
                ...
                document.cookie.split(";").map(function(v) {
                    var key = old_antimatterx.toBase58.decode(v.split("=")[0]);
                    if (!key.indexOf(thisURL)) arr.push(key.replace(thisURL, ""));
                });
            };
            return arr;
        ...
        removeSaveData: function(key) { // 指定したキーのデータを削除
            var SaveKey = old_antimatterx.makeSaveKey(key);
            if (!SaveKey) return false;
            if (window.localStorage) window.localStorage.removeItem(SaveKey);
            else document.cookie = old_antimatterx.toBase58.encode(SaveKey) + "=; max-age=0";
        },
        save: function(key, val) { // 文字列を保存
            var SaveKey = old_antimatterx.makeSaveKey(key);
            if (!SaveKey) return false;
            if (window.localStorage) window.localStorage.setItem(SaveKey, val);
            else document.cookie = old_antimatterx.toBase58.encode(SaveKey) + "=" + old_antimatterx.toBase58.encode(val);
        },
        load: function(key, callback) { // 保存した文字列を読み込んでcallbackの引数に渡す
            ...
            if (window.localStorage) {
                ...
                var encodedKey = old_antimatterx.toBase58.encode(SaveKey),
                    i = document.cookie.indexOf(encodedKey + "=") + encodedKey.length + 1;
                if (i === -1) return false;
                data = old_antimatterx.toBase58.decode(document.cookie.slice(i).split(";")[0]);
            };
            callback(data);
        },
        //--------------------------------------------------
        ...
        getFontSize: function(elm) { // 要素のフォントサイズを取得する
            return Number(old_antimatterx.getCSS(elm).fontSize.slice(0, -2)) + 1;
        },
        getRGB: function(color) { // color文字列をRGBの配列にして返す
            var elm = $("<div>").css("color", color).appendTo("body"),
                m = old_antimatterx.getCSS(elm).color.match(/[0-9]+/g);
            elm.remove();
            if (!m) return false;
        ...
        setWallpaper: function(url, cover) { // 壁紙を設定する
            var p = old_antimatterx.init(cover, {
                    color: "white",
                    opacity: 0.8
                }),
                colors = old_antimatterx.getRGB(p.color),
                elm = $("<div>").css({
                    zIndex: "-114514",
        ...
        addInputText: function(parentNode, param) { // 文字列入力欄を追加
            var p = old_antimatterx.init(param, {
                id: "", // HTML
                class: "", // HTML
                title: "", // タイトル
                placeholder: "", // 説明文
            ...
            if (p.textarea && old_antimatterx.getBrowser() === "Google Chrome") { // https://qiita.com/okyawa/items/8c7bee52b203f6956d44
                var str = elm.val();
                elm.focus();
                elm.val("");
                elm.val(str);
            ...
            function change() {
                var v = elm.val();
                if (p.trim) v = v.trim();
                if (p.hankaku) v = old_antimatterx.toHan(v);
                var re = p.change(v); // 引数に文字列入力欄の値を渡す
                if (old_antimatterx.judgeType(re, "String")) v = re;
                elm.val(v);
                if (p.save) old_antimatterx.save(p.save, v);
            };
            elm.on("change", change);
            old_antimatterx.try(change);
            return elm;
        },
        addInputNumber: function(parentNode, param) { // 数値入力欄を追加
            if (old_antimatterx.judgeType(param, "Object")) {
                ["value", "min", "max"].forEach(function(v) {
                    if (old_antimatterx.judgeType(param[v], "String")) param[v] = Number(param[v]);
                });
            };
            var p = old_antimatterx.init(param, {
                id: "", // HTML
            ...
            function change() {
                var n = Number(old_antimatterx.toHan(elm.val().trim()).replace(/[^0-9\.\-\+]/g, ""));
                if (isNaN(n)) n = 0;
                if (n < p.min) n = p.min;
                else if (n > p.max) n = p.max;
                if (p.int) n = Math.floor(n);
                var re = p.change(n); // 引数に数値入力欄の値を渡す
                if (old_antimatterx.judgeType(re, "Number")) n = re;
                var v = String(n);
                elm.val(v);
                if (p.save) old_antimatterx.save(p.save, v);
            };
        ...
        addInputBool: function(parentNode, param) { // ON/OFFボタンを追加
            var p = old_antimatterx.init(param, {
                ...
                flag = !!p.value,
                elm = old_antimatterx.addButton(parentNode, {
                    title: p.title,
                    click: function() {
                        flag = !flag;
                        change();
            ...
            if (p.save) old_antimatterx.load(p.save, function(v) {
                flag = (v === "1");
                change();
            });
            ...
            function change() {
                ...
                if (old_antimatterx.judgeType(re, "Boolean")) flag = re;
                elm.css("background-color", flag ? "orange" : "gray");
                check.prop("checked", flag);
                if (p.save) old_antimatterx.save(p.save, flag ? "1" : "0");
            };
            old_antimatterx.try(change);
            return elm;
        },
        ...
        addHideArea: function(parentNode, param) { // ボタンで表示を切り替えられる非表示エリアを追加
            var p = old_antimatterx.init(param, { // addInputBool参照
                    id2: "", // HTML
                    class2: "", // HTML
                    speed: 300, // 表示スピード[秒]
                    elm: $("<div>") // 非表示エリアにする要素
            ...
            old_antimatterx.addInputBool(front, p);
            return h;
        },
    ...
    function _setAttr(p, elm) { // 要素の属性の設定
        ...
        elm.attr("placeholder", p.placeholder).css({
            maxWidth: "95%",
            minWidth: old_antimatterx.getFontSize() * 5,
    ...
    function _setResize(p, elm, parentNode, func) { // リサイズ処理
        function resize() {
            ...
            function mostLongLine(str) { // 文字列の中で最も長い行の文字数を取得
                return old_antimatterx.max(str.split("\n").map(function(v) {
                    return v.length;
                }));
            };
            var fontSize = old_antimatterx.getFontSize(),
    ...
    function _setCommonInput(p, elm, parentNode) { // 標準的な入力欄の設定
        ...
        if (p.save) {
            old_antimatterx.load(p.save, function(v) {
