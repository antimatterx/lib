(function(window, undefined) {
    ...
    var antimatterx = {
        ...
        download: { // ファイルの保存
            ...
            image: function(title, dataURI) { // 画像を保存
                if (!antimatterx.judgeType(title, "String") ||
                ...
                antimatterx.download.file(m[1].split("/")[1], new Blob([cont], {
                    type: m[1]
                }));
            }
        },
        ...
        removeSavaData: function(key) { // 指定したキーを削除
        ...
        load: function(key, callback) { // 保存した文字列を読み込んでcallbackの引数に渡す
            ...
            if (window.localStorage) {
                ...
                var encodeKey = antimatterx.toBase58.encode(saveKey);
        ...
        addInputRange: function(parentNode, param) { // 数値入力範囲バーを追加
            ...
            function change() {
                ...
                if (isNaN(n)) n = 0;
                var re = p.change(n); // 引数に範囲バーの数値を渡す
                if (antimatterx.judgeType(re, "Number")) n = re;
                var v = String(n);
                elm.val(v);
                if (p.save) antimatterx.save(p.save, v);
        ...
        addInputButton: function(parentNode, param) { // ボタンを追加
            ...
            var elm = $("<button>").text(p.title).on("click", function(e) {
                p.click(e);
            }).appendTo(parentNode);
        ...
        addInputBool: function(parentNode, param) { // ON/OFFボタンを追加
            ...
            var flag = !!p.value;
            var elm = antimatterx.addInputButton(parentNode, {
                title: p.title,
                click: function() {
                    flag = !flag;
                    change();
        ...
        toImage: { // 文字列を画像化する
            ...
            decode: function(callback) { // 画像を文字列にデコードしてcallbackの引数に渡す
                var inputFile = $("<input>", {
                    type: "file"
        ...
        //--------------------------------------------------
        // その他
        try: function(func) { // 失敗しても止まらないようにfuncを実行する
            try {
                func();
            } catch (e) {
                console.error(e);
        ...
        getIP: function(callback) { // IPアドレス等の情報を取得してcallbackの引数に渡す
            $.get("https://ipinfo.io/?callback=a").done(function(d) {
                var m = d.match(/{.*?}/);
                if (!m) return;
