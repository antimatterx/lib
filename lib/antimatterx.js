/*!
 * AntimatterX JavaScript Library v1.0.5
 * https://github.com/AntimatterX/lib/blob/main/lib/antimatterx.js
 *
...
(function(globalObject, undefined) {
    ...
    var lib = {
        ...
        //--------------------------------------------------
        // データ保存関連
        makeSaveKey: function(key, uri) { // URIごとに保存する領域を分けるためのキーを作成する
            key = lib.initType(key, "");
            uri = lib.parseURI(uri).address;
            if (key === "" || uri === "") return "";
            return uri + "|" + key;
        },
        getSaveKeys: function(uri) { // 保存されているキーを配列で取得する
            uri = lib.parseURI(uri).address;
            var saveArea = uri + "|";
        ...
        makeQuery: function(param) { // クエリパラメータ文字列を作成する
        ...
        parseQuery: function(str) { // クエリパラメータ文字列を解析して連想配列形式で返す
            var obj = {};
        ...
        parseDomain: function(uri) { // URIからドメインを解析して配列で返す
            var m = (lib.initType(uri, context.href).match(/^[^:]+:\/\/([^\/\.]+\.[^\/]+)/) || []);
            return m.length > 0 ? m[1].split(".") : [];
        },
        parseURI: function(uri) { // URIを解析する
            var m = (lib.initType(uri, context.href).match(/^(([^:]+:)(\/\/)?([^\?#]+))(\?[^#]*)?(#.*)?$/) || []),
                isURI = (m.length > 0);
            return {
                ...
                protocol: m[2] || "", // プロトコル
                domainname: lib.parseDomain(m[1] || "").join(".") || "", // ドメイン文字列
                ...
                params: lib.parseQuery(m[5] || ""), // クエリパラメータ
                fragment: m[6] || "" // フラグメント
            };
        },
        ...
        extractURI: function(str) { // 文字列からURIを抽出する
            return lib.initType(str, "").match(/(([^:]+:)(\/\/)?([^\?#]+))(\?[^#]*)?(#.*)?/g) || []
        },
