    ...
    var lib = {
        ...
        hello: function() { // ライブラリの説明を返す
            return Object.keys(lib).map(function(k) {
                return [
                    k,
                    (String(lib[k]).match(/function.*?(\(.*?\))/) || [])[1] || Object.prototype.toString.call(lib[k]),
                    String(lib[k]).match(/\/\/.*/) || ""
                ].join(" ");
            }).join("\n");
        },
        ...
        parseURL: function(url) { // URLを解析する
            ...
            return {
                ...
                params: (function() { // URLクエリパラメータ
                    var obj = {};
                    (m[5] || "").slice(1).split("&").forEach(function(v) {
                        var q = v.split("=");
                        if (q[0] === "") return;
        ...
        _recursiveStringize: function(x, _key) { // 再帰的にオブジェクトを文字列にして返す
            ...
            else if (lib.isType(x, ["Number", "RegExp", "Boolean", "Error"])) return String(x); // 数値、正規表現、真偽値、エラー
